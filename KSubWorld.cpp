...
KSubWorld::KSubWorld()
{
	...
	m_nCellWidth = 32;// defLOGIC_CELL_WIDTH;
	m_nCellHeight = 32;// defLOGIC_CELL_HEIGHT;
	m_dwCurrentTime = 0;

	m_SubWorldID = -1;
	nWorldMapInfo[STR_MAP_MASTER] = "STR_MAP_MASTER";
	nWorldMapInfo[STR_MAP_ZTONGNAME] = "STR_MAP_MASTER";
	nWorldMapInfo[STR_MAP_TGONGNAME] = "STR_MAP_MASTER";
	nWorldMapInfo[STR_MAP_TSHOUNAME] = "STR_MAP_MASTER";
	...
	m_nCurNpcNum = 0;	  //当前波数
	m_nCurNum = 0;      //每波的怪物数
	m_nWeather = 255;// WEATHERID_NOTHING;
...
void KSubWorld::ObjChangeRegion(int nSrcRnidx, int nDesRnIdx, int nIdx)
{
	...
	{
		SubWorld[Object_tt[nIdx].m_n_SubWorldID].m_Region[nSrcRnidx].RemoveObj(nIdx);
		//		ObjSet.Remove(nIdx);
		return;
	}
